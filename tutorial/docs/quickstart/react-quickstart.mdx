...
## First step - create a Membrane Client instance
...
### We need a hook that will set up and send your screen track
...
export const App = () => {
  ...
  const jellyfishClient = client.useSelector(
    (snapshot) => snapshot.connectivity.client
  );
...
You should now see your screen recieced to each connected client on the dashboard. You can add another participant to check this out! 
