...
![Finished app](./img/react-finished.gif)
...
<Tabs>
  ...
  <TabItem value="react" label="React">
    In order for this module to work you'll need to add our `react-client-sdk` and `browser-media-utils` packages This
    is necessary to create and connect Jellyfish Client.

```bash npm2yarn
npm install https://github.com/jellyfish-dev/react-client-sdk @jellyfish-dev/browser-media-utils
```
...
Then rename your index.css file to daisyui.css and add configs:

```css title="daisyui.css"
@tailwind base;
@tailwind components;
@tailwind utilities;
...
Import the CSS file in your main.tsx file:

```tsx title="main.tsx"
import "./daisyui.css";
```

Finally, add daisyUI to the tailwind.config.js:
...
You can also use our CSS Style Sheet for this project, available
[here](https://github.com/Crackhoff/jellyfish-clients-tutorials/blob/main/JellyfishDashboardReact/JellyfishDashboard/src/styles.css)
...
that will allow us to enter the peer token and a button which
...
add some logic to send our tracks to the server and recieve tracks from others.
